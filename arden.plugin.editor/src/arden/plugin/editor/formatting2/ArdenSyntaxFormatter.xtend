...
class ArdenSyntaxFormatter extends AbstractFormatter2 {
	...
	def dispatch void format(mlms mlms, extension IFormattableDocument document) {
		...
		for (P_mlm p_mlms : mlms.getP_mlms()) {
			p_mlms.format;
		}
	}

	def dispatch void format(P_mlm p_mlm, extension IFormattableDocument document) {
		// TODO: format HiddenRegions around keywords, attributes, cross references, etc. 
		p_mlm.getMaintainance_category.format;
		p_mlm.getLibrary_category.format;
		p_mlm.getKnowledge_category.format;
	}
